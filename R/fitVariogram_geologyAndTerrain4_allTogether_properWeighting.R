# fitVariogram_geologyAndTerrain4_allTogether_properWeighting.R
# 
...
source("R/fitVariogram_weighting.R")
...
pickVariogramVersion <- function(vgName) {
  ...
  return(list(psill=psill, nugget=nugget, kappa=kappa, range=range))
}
...
for(i in 1:length(varVersions)) {
  ...
  subsetList <- switch(MODEL,
                       AhdiAK_noQ3_hyb09c = c("noQ3noCanterbury", "noQ3noMcGann","McGann"),
  ...
  weightingList <- switch(MODEL,
                          AhdiAK_noQ3_hyb09c = c(0.5,  0.25, 0.25),
                          YongCA_noQ3        = c(0.5, 0.5) # B subsets: all cat 15 and 16 removed. See note in functions.R for more detail
  )
  ...
  subsetNameList <- switch(MODEL, # these are the labels used in the publication table.
                           AhdiAK_noQ3_hyb09c = c("GS1",  "GS2",  "GS3"),
                           YongCA_noQ3        = c("TS1",  "TS2"))
  
  
  varListForWeighting <- list()
  vgdfAll <- data.frame()
  vgmTheo_auto_ALL_forReal <- data.frame()
  for(modelSubsetNo in 1:length(subsetList)) { 
    ...
    varAllRes  <- var(res)
    res_norm_1 <- res / stdv                          # NOPE
    res_norm_2 <- res_norm_1 / var(res_norm_1)        # NOPE
    ...
    if(nrow(dat_subset)>=2) {  # need a minimum of 1 pair i.e. 2 datapoints for the empirical variogram.
      ...
      sn                <- subsetNameList[modelSubsetNo]
      vgdf$subsetName   <- rep(sn, nrow(vgdf))
      binLabelHt <- labHt - 0.5*(modelSubsetNo-1)
      vgdf$binLabelHt   <- rep(binLabelHt, nrow(vgdf))
      print(binLabelHt)
    ...
    vgmTheo_auto_ALL$subsetName <- rep(subsetNameList[modelSubsetNo], nrow(vgmTheo_auto_ALL))
    ...
    # save unique fit variograms for each subset:
    # 
    variogram <- fitVarAuto_ALL
    save(variogram, file = paste0("Rdata/variogram_", MODEL, "_dat_", modelSubset, ".Rdata"))
      
    varListForWeighting[[modelSubsetNo]] <- variogram
    vgdfAll <- rbind(vgdfAll,vgdf)
    vgmTheo_auto_ALL_forReal <- rbind(vgmTheo_auto_ALL_forReal, vgmTheo_auto_ALL)
  }
  ...
  if(plotErrorBars) {
    ...
    evp <- evp + geom_errorbar(
      data=vgdfAll, aes(x=dist/1000, ymin=boot025, ymax=boot975, col=subsetName),
      width=0.001*0.015*cutoffDist)
  }
  
  evp <- evp + geom_point(data = vgdfAll, aes(x=dist/1000, y=gamma,col=subsetName))
  
  if(plotTheoreticalVariogram) {
    evp <- evp + geom_line(data = vgmTheo_auto_ALL_forReal, aes(x=dist/1000,y=gamma, col=subsetName))
  }
  if(includeBinNos) {
    evp <- evp + geom_text(data = vgdfAll, aes(x=dist/1000, label=np, y=binLabelHt, col=subsetName), 
                           size = 2, angle = 90)
  }
  ...
  fitVarAuto_ALL2   <- fitVariogram_weighting(varListForWeighting, weightingList)
  ...
  print(sprintf("Variogram %s",vgName))
  print(sprintf("Effective range: %f",effectiveRangeAll))
        
  evp <- evp + xlab("distance (km)") + ylab("V(u)")
  ...
  # manual legend
  valList <- setNames(object=colorList, nm=subsetNameList)
  evp <- evp +
  ...
  ggsave(filename = sprintf("out/plots/vgram_avg_%s_%s_cut%03.0fkm_R%d.png",
                            switch(MODEL, AhdiAK_noQ3_hyb09c = "GEOLOGY",
                                   YongCA_noQ3 = "TERRAIN"),
                            vgName, 
                            cutoffDist/1000,
