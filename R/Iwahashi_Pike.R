...
# (Ended up being closer to 30 minutes)
#
# If I apply a 100m resolution resample (will most likely),
# then this becomes just a couple of minutes.
...
for (numCats in c(8,12,16)) {
  ...
  # Canterbury:
  # slpFileName <- "~/big_noDB/topo/terrainCats/slp_si_00070003.tif"
  # aspFileName <- "~/big_noDB/topo/terrainCats/asp_si_00070003.tif"
  # TPIfileName <- "~/big_noDB/topo/terrainCats/TPI_si_00070003.tif"
  # TRIfileName <- "~/big_noDB/topo/terrainCats/TRI_si_00070003.tif"
  # rufFileName <- "~/big_noDB/topo/terrainCats/ruf_si_00070003.tif"
  # medFileName <- "~/big_noDB/topo/terrainCats/med_si_00070003.tif"
  # txkFileName <- "~/big_noDB/topo/terrainCats/txk_si_00070003.tif" # texture (kevin)
  # txtFileName <- "~/big_noDB/topo/terrainCats/txt_si_00070003.tif" # texture (Iwahashi & Pike)
  # cvxFileName <- "~/big_noDB/topo/terrainCats/cvx_si_00070003.tif" # local convexity (Iwahashi & Pike)
  # outFileName <- sprintf("~/big_noDB/topo/terrainCats/IwahashiPike_si_00070003_%02d.tif",
  #                        numCats)

  
  # All NZ:
  slpFileName <- paste0("~/big_noDB/topo/terrainCats/slp_NZ_", DEMresample, ".tif")
  aspFileName <- paste0("~/big_noDB/topo/terrainCats/asp_NZ_", DEMresample, ".tif")
  TPIfileName <- paste0("~/big_noDB/topo/terrainCats/TPI_NZ_", DEMresample, ".tif")
  TRIfileName <- paste0("~/big_noDB/topo/terrainCats/TRI_NZ_", DEMresample, ".tif")
  rufFileName <- paste0("~/big_noDB/topo/terrainCats/ruf_NZ_", DEMresample, ".tif")
  medFileName <- paste0("~/big_noDB/topo/terrainCats/med_NZ_", DEMresample, ".tif")
  txkFileName <- paste0("~/big_noDB/topo/terrainCats/txk_NZ_", DEMresample, ".tif") # texture (kevin)
  txtFileName <- paste0("~/big_noDB/topo/terrainCats/txt_NZ_", DEMresample, ".tif") # texture (Iwahashi & Pike)
  cvxFileName <- paste0("~/big_noDB/topo/terrainCats/cvx_NZ_", DEMresample, ".tif") # local convexity (Iwahashi & Pike)
  outFileName <- sprintf("~/big_noDB/topo/terrainCats/IwahashiPike_NZ_%03dm_%02d.tif",
                         DEMresample, numCats)
  ...
  # DEM00    <- raster("~/big_noDB/topo/DEM_all_NZGD00.tif") # original resolution: 25m
  DEM00    <- raster("~/big_noDB/models/DEM_all_NZGD00_resampled.tif") # resampled: 100m (water is NA!)
  
  
  if(0) { # Canterbury crop
    extents <- extent(
      1470000,
      1620000,
      5100000,
  ...
  if (regenerate) {
    ...
    ###############################################################################################
    # Other options for raster::terrain() - not used
    #
    # asp <- terrain(x=DEM,
    #                opt = "aspect",
    #                unit = "degrees",
    ...
    # TPI <- terrain(x=DEM,
    #                opt = "TPI",
    #                filename = TPIfileName)
    #
    # TRI <- terrain(x=DEM,
    #                opt = "TRI",
    #                filename = TRIfileName)
    #
    # ruf <- terrain(x=DEM,
    #                opt = "roughness",
    #                filename = rufFileName)
    ...
    # Played with this but it implicitly includes a measure of local relief and therefore probably not ideal accoridng to Iwahashi & Pike.
    # textureK <- focal(x = peaksAndPits,       w=window, fun=sum, na.rm=T, filename=txkFileName, overwrite=T)
...
pp <- raster(x="~/big_noDB/topo/terrainCats/IwahashiPike_NZ_100m_08.tif")
qq <- raster(x="~/big_noDB/topo/terrainCats/IwahashiPike_NZ_100m_12.tif")
rr <- raster(x="~/big_noDB/topo/terrainCats/IwahashiPike_NZ_100m_16.tif")
...
writeRaster(x   = Vs30_YongEtAl,
      filename  = "~/big_noDB/models/Yong2012_Cali_Vs30.tif",
      format    = "GTiff",
      overwrite = T
      )
...
# look at ratio of geology to terrain:

geo <- raster(x = "~/big_noDB/models/geo_NZGD00_allNZ_AhdiAK.tif")
geoC <- crop(geo, Vs30_YongEtAl)

geoCR <- resample(geoC, Vs30_YongEtAl)

png("out/maps/geoVs30.png")
plot(geoCR, col=modelCol, zlim=c(0,1550))
dev.off()


ratio <- log(geoCR/Vs30_YongEtAl)
rbPal <- RColorBrewer::brewer.pal(n=9,name = "RdBu")
png("out/maps/geo_Yong_ratio.png")
  plot(ratio, col=rbPal, zlim=c(-1,1), main="Ratio (geo/Yong)")
dev.off()
