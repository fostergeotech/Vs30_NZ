# fitVariogram_geologyAndTerrain3_allTogether_justNQNC.R
# 
# 20181010 - copied this file from fitVariogram_geologyAndTerrain3_allTogether
# 
...
# 
# This is mostly the same as previous version but I am defining new variograms v8 and v9
# as those from JUST noQ3noCanterbury.
# 
rm(list=ls())
setwd("~/VsMap")
library(gstat)
...
# 
# v8: GEO, JUST noQ3noCanterbury.
# v9: TER, JUST noQ3noCanterburyB.
...
for(i in 1:length(varVersions)) {
  ...
  for(cutoffD in 1:length(cutoffDists)) {
    ...
    for(normResFlag in normResFlags) {
      ...
      for(modelSubsetNo in 1:length(subsetList)) { 
        ...
        # Selecting the best variogram for each model.
        if( (MODEL == "AhdiAK_noQ3_hyb09c" & modelSubset == "noQ3noCanterbury"  & binningScheme == 14) |
            (MODEL == "YongCA_noQ3"        & modelSubset == "noQ3noCanterburyB" & binningScheme == 16)) {
          thisIsTheVariogramWeWillUse <- fitVarAuto_ALL
        }
      ...
      # No longer using "average" variogram so do not plot.
      # if(plotTheoreticalVariogram) {
      ...
      ggsave(filename = sprintf("out/plots/NEWNEWNEW/vgram_all_%s_%s_cut%03.0fkm_R%d.png",
                                switch(MODEL, AhdiAK_noQ3_hyb09c = "GEOLOGY",
                                       YongCA_noQ3 = "TERRAIN"),
                                vgName, 
                                cutoffDist/1000,
  ...
  variogram <- thisIsTheVariogramWeWillUse
